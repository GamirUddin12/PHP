...
<table align="center" bgcolor="majenda" border="1" width="60%" height="50%">
	...
	<hr>
     <p><font face="arial" color="Green" size="+1">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Patient Released  from Hospital</font></p>
...
$sql_1 = "SELECT * FROM p_admission AS p, p_cured AS c WHERE p.p_id=c.p_id AND p.status = 0 AND p.p_dept= '$deptName[$i]'";
...
echo "<tr><th>name</th><th>age</th><th>gender</th><th>Dept</th><th>Bed No.</th><th>Admission date</th><th>Release date</th><th>Bill</th></tr>";
    if($count == 0){
		...
		$p_release_date=$row['p_release_date'];
		$dept=$row['p_dept'];
		$bed_no=$row['p_bed_no'];
		$p_bill=$row['p_bill'];
		
		echo "<tr><td>$name</td><td>$age<td>$gender</td><td>$dept</td><td>$bed_no</td><td>$admission_date</td><td>$p_release_date</td><td>$p_bill</td></tr>";
